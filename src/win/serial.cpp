...
struct device {
  ...
  bool waiting_on_read;      // Used to prevent creation of new read operation
...
device_s device_construct(const char* port, int32_t baudrate) {
  ...
  if ( baudrate != 230400 ) {
    throw error{"baudrate is not supported."};
  }
...
void device_read(device_s serial, void* to, int32_t len) {
  ...
  if ( !serial->waiting_on_read ) {
    ...
    if ( !ReadFile(serial->h_comm, (unsigned char*)to, dw_num_to_read,
                &dw_num_read, &(serial->os_reader)) ) {
  ...
  if ( serial->waiting_on_read ) {
    dwRes = WaitForSingleObject(serial->os_reader.hEvent,
        serial->read_timeout_millis);
    switch ( dwRes ) {
    // Read completed.
    case WAIT_OBJECT_0:
      if ( !GetOverlappedResult(serial->h_comm, &(serial->os_reader),
            &dw_num_read, FALSE) ) {
  ...
  NEO_ASSERT(f_result && "reliable read failed to read requested number of bytes.");
  if ( !f_result )
    throw error{"reading from serial device failed."};
}
...
void device_write(device_s serial, const void* from, int32_t len) {
  ...
  NEO_ASSERT(f_result && "reliable write failed to write requested number of bytes.");
  if ( f_result == false )
